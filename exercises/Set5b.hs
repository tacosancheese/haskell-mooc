...
treeMax Empty = 0
treeMax (Node v l r) = max v (max lv rv)
  where lv = treeMax l
        rv = treeMax r
...
set _ _ Empty                 = Empty
set [] v (Node x l r)         = Node v l r
set p v (Node x Empty Empty)  = Node x Empty Empty
set (p:ps) v (Node x l r)     = case p of
  StepL -> (Node x (set ps v l) r)
  StepR -> (Node x l (set ps v r))
