...
longest (x:y:xs)
  ...
  | length y > length x = longest (y:xs)
  | otherwise = if x > y then longest (y:xs) else longest (x:xs)
...
winner scores player1 player2
  | s1 >= s2  = player1
  | otherwise = player2
  where s1 = Map.findWithDefault 0 player1 scores
...
freqs xs = Map.fromList [(i, count i xs) | i <- nub xs]
  where count a b = (length . filter (==a)) b
...
maxIndex xs = fst (maximumBy (\a b -> compare (snd a) (snd b)) (assocs xs))
