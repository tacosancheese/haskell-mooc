...
joinToLength l strings = [c | x <- strings, y <- strings, let c = x++y, length c == l]
...
multiCompose fs x = foldr (\f y -> f y) x fs
...
interpreter commands = move 0 0 commands
  where
    move x y [] = []
    move x y ("up" : cs) = move x (y+1) cs
    move x y ("down" : cs) = move x (y-1) cs
    move x y ("left" : cs) = move (x-1) y cs
    move x y ("right" : cs) = move (x+1) y cs
    move x y ("printX" : cs) = show x : move x y cs
    move x y ("printY" : cs) = show y : move x y cs
