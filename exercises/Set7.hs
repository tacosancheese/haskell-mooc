...
add item (Set xs) = if elem item xs
  then (Set xs)
  else Set (sort (item:xs))
...
data State = Start | Error | Finished | Eggs | Sweet | Flour | Dough | Mixed
  deriving (Eq,Show)

step :: State -> Event -> State
step Start    AddEggs   = Eggs
step Eggs     AddFlour  = Flour
step Eggs     AddSugar  = Sweet
step Flour    AddSugar  = Dough
step Sweet    AddFlour  = Dough
step Dough    Mix       = Mixed
...
passwordAllowed str (ContainsSome rule)   = length (filter (\x -> elem x rule) str) > 0
passwordAllowed str (DoesNotContain rule) = not $ passwordAllowed str (ContainsSome rule)
passwordAllowed str (And r1 r2)           = all (\x -> passwordAllowed str x) (r1:r2:[])
passwordAllowed str (Or r1 r2)            = any (\x -> passwordAllowed str x) (r1:r2:[])
