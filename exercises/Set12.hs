...
fmap2 f xs = fmap (\x -> fmap f x) xs

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f xs = fmap2 (\x -> fmap f x) xs
...
count x foldable = foldl (\acc y -> acc + (value x y)) 0 foldable
  where value x y = if x == y then 1 else 0
...
inBoth f1 f2 = filter (\x -> elem x l1) l2
  where l1 = (toList f1)
        l2 = (toList f2)
...
instance Foldable List where
  foldr _ value Empty       = value
  foldr f value (LNode x n) = f x (foldr f value n)
...
instance Foldable TwoList where
  foldr _ value TwoEmpty        = value
  foldr f value (TwoNode x y n) = f x ((f y) (foldr f value n))
